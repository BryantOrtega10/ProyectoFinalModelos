...
def crear():
    response_body = copy.deepcopy(RESPONSE_BODY_DEFAULT)
    status_code = HTTPStatus.OK
    ciu_v_nombre = request.form.get("ciu_v_nombre")
...
def modificar():
    ...
    id = request.form.get("ciu_i_id")
    nombre = request.form.get("ciu_v_nombre")
    if id != "" and nombre != "" and id != None and nombre != None:
        ciudad_mod = modificar_ciudad(id, nombre)
        if ciudad_mod != None:
            response_body["message"] = "Ciudades modificada correctamente!"
